...
// Edge is a struct that contains the name of the edge and two vertices as characters -- 
...
// •char* vertices( ): Returns a character array (via character pointer) to all 
// the vertices of the graph (arranged in the order of how they are added).
...
// •char* endVertices(e): Returns a character array (via character pointer) 
// containing the two endpoint vertices of edge e. Since the graph is directed, 
...
// •char opposite(v, e): For edge e incident to vertex v, returns the other vertex of the edge; 
// returns "-" if e is not incident to v.
...
// all incoming edges to vertex v. This will be arranged in such a way 
// that incoming edges from vertices that were added first will be listed first as well.
